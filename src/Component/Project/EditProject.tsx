...
function EditProject({ isOpen, onClose, onProjectUpdated, project }: EditProjectProps) {
  ...
  // Pre-fill form data when project prop changes
  useEffect(() => {
    if (project && isOpen) {
      ...
      if (Array.isArray(project.teamMembers)) {
        teamMemberIds = project.teamMembers.map(member => member.userId || '').filter(Boolean);
      } else if (Array.isArray(project.members) && typeof project.members[0] !== 'number') {
        teamMemberIds = (project.members as TeamMember[]).map(member => member.userId || '').filter(Boolean);
      }
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        await api.patch(`/projects/${projectId}`, submitData);
        resetForm();
        onProjectUpdated?.();
        onClose();
      } catch (err: any) {
  ...
  return (
    <>
      {/* Loading Overlay */}
      {loading && <Loading variant="overlay" text={UPDATING_PROJECT} />}
